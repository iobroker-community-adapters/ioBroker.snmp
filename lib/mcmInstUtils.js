...
class mcmInstUtils {
    ...
    async _convInstance(pInstanceRow) {
        this.logger.debug("mcmInstUtils/convInstance - process instance" + pInstanceRow.id);
        ...
        if (cfgVers == 0 || OIDs) {
            if (OIDs && OIDs.length > 0) {
                this.logger.info("instance " + pInstanceRow.id + " will be migrated");
                ...
                this.logger.info("instance " + pInstanceRow.id + " has been migrated");
                this.doRestart = true;
                ...
                // write object
                await this.adapter.setForeignObjectAsync(pInstanceRow.id, pInstanceRow.value);
                this.logger.info("instance " + pInstanceRow.id + " provides no data to migrate");
            }
        } else if (cfgVers == '2.0') {
            // remove old configuration anyway
            delete native.OIDs;
            ...
            // write object
            await this.adapter.setForeignObjectAsync(pInstanceRow.id, pInstanceRow.value);
            this.logger.info("instance " + pInstanceRow.id + " already up to date");
        } else {
            this.logger.warn("instance " + pInstanceRow.id + " reports unknown config version '" + cfgVers + "'");
        };
    }

    async doUpgrade(pInstance) {
        const tmp = (typeof pInstance === 'undefined') ? '' : ('restricted to instance ' + pInstance);
        this.logger.debug("mcmInstUtils/doUpgrade - starting upgrade process" + tmp);
