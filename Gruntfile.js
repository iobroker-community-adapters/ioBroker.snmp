...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'Marcolotti';
    var email     = grunt.option('email')  || 'info@ct-j.de';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new snmp-oid name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/snmp-oid.png')) {
            fs.renameSync(__dirname + '/admin/snmp-oid.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/snmp-oid.html')) {
            fs.renameSync(__dirname + '/widgets/snmp-oid.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/snmp-oid/js/snmp-oid.js')) {
            fs.renameSync(__dirname + '/widgets/snmp-oid/js/snmp-oid.js', __dirname + '/widgets/snmp-oid/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/snmp-oid')) {
            fs.renameSync(__dirname + '/widgets/snmp-oid',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Snmp-oid/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Snmp-oid'
                        },
                        {
                            match: /Marcolotti/g,
                            replacement: author
